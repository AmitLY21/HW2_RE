...
public class Activity_Game extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (id.length() == this.steps.length) {
            ...
            while (true) {
                ...
                Log.d("pttt", "In while loop: iArr[" + i + "] = " + iArr[i]);
                i++;
            }
        }
        findViews();
    ...
    /**
     *
     * @param direction is the arrow button index in the arrows array
     * In order to win you will need to press the arrow which his index is matching the steps[currentLevel]
     */
    public void arrowClicked(int direction) {
        Log.d("pttt", "arrowClicked: goodToGo -> " + this.goodToGo + " , direction -> " + direction + " , steps[currentLevel] -> " + this.steps[this.currentLevel]);
    ...
    private void findViews() {
        ...
        Log.d("pttt", "Arrows: left -> 0 , right -> 1 , up -> 2 , down -> 3" );
    }
}
